...
const EmployeeLoan = () => {
  ...
  const calculateLoan = () => {
    ...
    for (let i = 1; i <= installments; i++) {
      const interest = interestType === 'withInterest' ? remaining * rate : 0;
      const principalPayment = Math.min(parseFloat(installmentAmount) - interest, remaining);
          ...
          <div className="mb-8 p-6 bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-lg shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Interest Rate (%)
                  {interestType === 'withInterest' && <span className="text-red-500 ml-1">*</span>}
                </label>
                <input 
                  type="number" 
              ...
              <table className="min-w-full bg-white">
                <thead>
                  <tr className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
                    ...
                    <th className="py-4 px-6 text-right font-semibold">Outstanding</th>
                    <th className="py-4 px-6 text-right font-semibold">Capital Repayment</th>
                    <th className="py-4 px-6 text-right font-semibold">Interest</th>
                    <th className="py-4 px-6 text-right font-semibold">Installment</th>
                    <th className="py-4 px-6 text-right font-semibold">Balance</th>
                        ...
                        <div className="flex flex-col items-center space-y-4">
                          ...
                          <div className="text-gray-500">
                            <p className="text-lg font-medium mb-1">
                              {isCalculated ? 'No loan details available' : 'Ready to Calculate'}
                            </p>
                            <p className="text-sm">
                              {isCalculated ? 'Please check your input values' : 'Enter loan details and click Calculate to view schedule'}
                            </p>
                          </div>
                        </div>
                      </td>
